...
```
bun .cursor/tools/aidlc-state.ts reuse-artifact reverse-engineering --decision keep --artifacts "<codekb-path output>" [--repo <repo>] [--single]
```
...
```
bun .cursor/tools/aidlc-utility.ts codekb-snapshot --repo <repo> --paths <source paths> --json
```
